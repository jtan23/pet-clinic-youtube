...
REM Create ACM certificate by calling template
aws cloudformation deploy ^
    --stack-name acm-dns ^
    --template-file 41-cfn-acm-dns.yaml  ^
    --parameter-overrides Environment=test DomainName=test.jtan23.net ^
    --capabilities CAPABILITY_IAM

REM Go to ACM console
REM Select the newly created certificate
REM Expand the domain
REM Click Create Record in Route 53

REM Create External DNS
start "Create External DNS" setup-04-external-dns.bat

REM (Optional) Check https working OK
kubectl apply -f 03-testing-external-dns.yaml

REM (Optional) Remove testing
kubectl delete -f 03-testing-external-dns.yaml
