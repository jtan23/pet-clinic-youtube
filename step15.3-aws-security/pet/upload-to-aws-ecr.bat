...
REM Build image
docker build -t pet:latest --build-arg DB_ENDPOINT=%MYSQL_ENDPOINT% .

REM (Optional) Create ECR repository if not already existed
aws ecr create-repository --repository-name pet

REM Tag the image for upload
docker tag pet:latest 291148202449.dkr.ecr.ap-southeast-2.amazonaws.com/pet:latest
...
docker push 291148202449.dkr.ecr.ap-southeast-2.amazonaws.com/pet:latest
