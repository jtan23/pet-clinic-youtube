...
REM Build image
docker build -t owner:latest --build-arg DB_ENDPOINT=%MYSQL_ENDPOINT% .

REM (Optional) Create ECR repository if not already existed
aws ecr create-repository --repository-name owner

REM Tag the image for upload
docker tag owner:latest 291148202449.dkr.ecr.ap-southeast-2.amazonaws.com/owner:latest
...
docker push 291148202449.dkr.ecr.ap-southeast-2.amazonaws.com/owner:latest
