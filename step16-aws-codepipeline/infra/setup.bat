...
REM Created Managed Policies, and IAM Service Accounts for our services in both test and prod namesapces. About 8 mins.
start "Create App Service Accounts" setup-11-app-service-account.bat
...
REM Create 'Dockerfile' and run 'upload-to-aws-ecr.bat' in all services, 
REM so that the below deployment can pick up the images from AWS ECR

REM Deploy applications to test namespace
...
kubectl describe service -n test app-service

REM We should be able to access the application here: http://petclinic.test.jtan23.net

REM (Optional) If not already done:
REM Create 'buildspec.yml', 'deployspec-test.yml', 'deployspec-prod.yml', 
REM 'k8s-service-deployment.yaml' and 'version' files in all services, 
REM and run 'upload-to-aws-codecommit.bat' in all services, 
REM so that the below CICD can build and deploy the services

REM Delete test applications
...
REM Create CICD
start "Create CICD" setup-21-cicd.bat

REM We should be able to access the application here: http://petclinic.test.jtan23.net
REM Once we manually approved all the prod deployments, here is the prod link: http://petclinic.jtan23.net
